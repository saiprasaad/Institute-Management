...
@Component({
  selector: 'app-feedbackcountadmin',
  templateUrl: './feedbackcountadmin.component.html',
  styleUrls: ['./feedbackcountadmin.component.css']
})
export class FeedbackcountadminComponent implements OnInit {
  ...
  constructor(private router:ActivatedRoute,private service:StaffdetailsService,private service1:ProfilesService,private fb:FormBuilder,private service2:FeedbackService,private route:Router) { 
   ...
   this.service.getStaffbysubcode(this.subcode,this.sec).subscribe(
      data=>
      {
        this.staffid=data.substring(0,5);
